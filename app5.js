...
// ================== NAVIGATION ==================
// Fungsi navigasi yang sudah disederhanakan dan universal
function showPage(pageId) {
  // Sembunyikan semua halaman dan hapus class 'active' dari semua tombol
  document.querySelectorAll(".page").forEach((p) => p.classList.remove("active"));
  document.querySelectorAll(".navbar button").forEach((b) => b.classList.remove("active"));

  // Tampilkan halaman yang dipilih dan berikan class 'active' pada tombolnya
  document.getElementById(pageId).classList.add("active");
  document.getElementById("tab-" + pageId.split("-")[1]).classList.add("active");
}

// Tambahkan event listener untuk semua tombol navigasi
document.addEventListener("DOMContentLoaded", function() {
  // Daftar semua halaman, termasuk "about-me"
  const pages = ["home", "about-me", "hash", "block", "chain", "ecc", "consensus"];
  
  pages.forEach((p) => {
    const tabButton = document.getElementById("tab-" + p);
    if (tabButton) {
      tabButton.onclick = () => showPage("page-" + p);
    }
  });
});
